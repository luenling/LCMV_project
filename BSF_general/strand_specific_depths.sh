...
cut -f 1,2 $( ls  Coverages/*_plus.depth | head -1 ) | cat <( echo -e "CHROM\tBPS")  - > Coverages/bsf_${RUN_ID}_all_plus.depths
...
cut -f 1,2 $( ls  Coverages/*_minus.depth | head -1 ) | cat <( echo -e "CHROM\tBPS")  - > Coverages/bsf_${RUN_ID}_all_minus.depths
...
cd Coverages
Rscript $(dirname $BASH_SOURCE)"/coverage_stranded.R" bsf_${RUN_ID}_all_plus.depths bsf_${RUN_ID}_all_minus.depths
cd ..
exit
