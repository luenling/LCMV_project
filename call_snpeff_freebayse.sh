...
echo cut -f 1-5 $1 \| $SNPEFF -c $SNPEFF_CONF -dataDir $SNPEFF_DATA lcmv -no-intergenic -no "INTRAGENIC" -no-downstream -no-upstream -stats ${FN}_snpeff_log.html - \> ${FN}_snpeff.vcf >> $LOGFILE
cut -f 1-5 $1 | $SNPEFF -c $SNPEFF_CONF -dataDir $SNPEFF_DATA lcmv -no-intergenic -no-downstream -no "INTRAGENIC" -no-upstream -stats ${FN}_snpeff_log.html - > ${FN}_snpeff.vcf

ES=$?
echo finished snpeff at `date` with exit state $ES >> $LOGFILE
[ $ES -eq 0 ] || exit $ES

java -jar $SNPSIFT extractFields ${FN}_snpeff.vcf CHROM POS REF "ANN[0].GENE" "ANN[0].ALLELE" "ANN[0].EFFECT" "ANN[0].AA" "ANN[1].ALLELE" "ANN[1].EFFECT" "ANN[1].AA" "ANN[2].ALLELE" "ANN[2].EFFECT" "ANN[2].AA" "ANN[3].ALLELE" "ANN[3].EFFECT" "ANN[3].AA" "ANN[4].ALLELE" "ANN[4].EFFECT" "ANN[4].AA" > ${FN}_snpeff.tab
