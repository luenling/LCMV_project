...
def merge_to_vcf(
    ...
    with open(tmpVcf, 'a') as outf:
        ...
        #print str(ref_seq_id_to_alignment_file)
        #print str(ref_seq_in_alignment_file)
            ...
            for ref_sequence in Bio.SeqIO.parse(inf, 'fasta'):
                ...
                for s in samplesToUse:
                    for row in util.file.read_tabfile(samp_to_isnv[sampleIDMatch(s)]):
                        # map ref->sample
                        #print str(s)
                        #print str(cm.mapChr(ref_sequence.id, s))
                        s_chrom = cm.mapChr(ref_sequence.id, s)
                        #if row[0] == s_chrom:
                        if row[0] == ref_sequence.id:
