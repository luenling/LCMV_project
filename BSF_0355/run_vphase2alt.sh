...
while read FN; do
	FNB=`basename $FN .bam`
	SAMP=$($SAMTOOLS view -H $FN | grep '^@RG' | sed "s/.*SM:\([^\t]*\).*/\1/g")
...
for i in $SAMPLES; do
  $SAMTOOLS faidx $REFGENOME L | python ${SCRIPTS}/change_consensus.py -p <( grep "^L.*snp"  BSF_*_${i}_sorted_viral_rh.tab  | cut -f 1,2,4 ) | sed 's/>L/>'$i'/g' >> L.fasta
  $SAMTOOLS faidx $REFGENOME S | python ${SCRIPTS}/change_consensus.py -p <( grep "^S.*snp" BSF_*_${i}_sorted_viral_rh.tab | cut -f 1,2,4 )  | sed 's/>S/>'$i'/g' >> S.fasta
done
...
if [[ -e $DPS ]];
then
    $BCFTOOLS annotate -a $DPS -c "+FORMAT/DP" all_out.vcf.gz > all_out_dps.vcf
    LVCF=all_out_dps
fi
